...
def add_consts_with_placeholder():
    ...
    """
    number_51 = tf.constant(5.1)
    number_10 = tf.constant(1.0)
    c3 = tf.placeholder(dtype = tf.float32)

    a1 = tf.add(number_51, number_10)
    af = tf.add(a1, c3)
    return af, c3
...
def my_perceptron(x):
    ...
    W = tf.get_variable(shape=[x], name= "Weight", initializer = tf.ones_initializer(), dtype = tf.float32)
    multiply_result = tf.multiply(i,W)
    out = my_relu(tf.reduce_sum(multiply_result))
    return i, out
...
def onelayer(X, Y, layersize=10):
    """
    Create a Tensorflow model for truncated_normal (i.e. single layer NN)
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    input_size = 784
    w1_shape = [input_size, hiddensize]
    w1 = tf.Variable(tf.truncated_normal(shape=w1_shape,stddev=0.1))
    b1_shape = [hiddensize]
    b1 = tf.Variable(tf.constant(0.1, shape=b1_shape))
    w2_shape = [hiddensize, outputsize]
    w2 = tf.Variable(tf.truncated_normal(shape=w2_shape,stddev=0.1))
    b2_shape = [outputsize]
    b2 = tf.Variable(tf.constant(0.1, shape=b2_shape))
    hide_1 = tf.nn.relu(tf.matmul(X, w1) + b1)
    logits = tf.matmul(hide_1, w2) + b2
    preds = tf.nn.softmax(logits)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    conv2 = tf.layers.conv2d(inputs=conv1,
                             ...
                             padding="SAME",
                             activation=tf.nn.relu)
    f = tf.reshape(conv2,[-1,28*28*outputsize])
    w_shape = [28 * 28 * convlayer_sizes[1], outputsize]
    w = tf.Variable(tf.truncated_normal(shape=w_shape,stddev=0.1))
    b_shape = [outputsize]
